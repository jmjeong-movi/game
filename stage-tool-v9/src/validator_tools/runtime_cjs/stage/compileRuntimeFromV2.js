...
exports.compileRuntimeFromV2 = compileRuntimeFromV2;
...
function compileRuntimeFromV2(stage, opt) {
    ...
    const enableStacks = !!(opt === null || opt === void 0 ? void 0 : opt.enableStacks) ||
        ((_a = stage.meta.layerCount) !== null && _a !== void 0 ? _a : 1) > 1 ||
        ((_d = (_c = (_b = stage.board) === null || _b === void 0 ? void 0 : _b.objects) !== null && _c !== void 0 ? _c : stage.objects) !== null && _d !== void 0 ? _d : []).some((o) => { var _a; return ((_a = o.layer) !== null && _a !== void 0 ? _a : 0) > 0; });
    const objects = ((_g = (_f = (_e = stage.board) === null || _e === void 0 ? void 0 : _e.objects) !== null && _f !== void 0 ? _f : stage.objects) !== null && _g !== void 0 ? _g : []);
    const id = stage.id;
    const meta = stage.meta;
    // ✅ colorCount는 "스테이지가 쓰는 색 개수"(툴 메타)로만 취급합니다.
    // 런타임 colorId는 항상 전역(0~11) 기준으로 유지해야 컬러/스프라이트가 절대 흔들리지 않습니다.
    const metaColorCount = (0, util_1.clamp)(Math.floor(meta.colorCount), 1, COLOR_MAX);
    ...
    const slotCount = (0, util_1.clamp)(Math.floor(meta.slotCount), MIN_SHOOTERS, 5);
    // ✅ waitLineCount: 대기 라인 수(기본 3). UnitLineRack에서 WaitingLine01~03 표시 수를 결정합니다.
    const waitLineCount = (0, util_1.clamp)(Math.floor((_h = meta.waitLineCount) !== null && _h !== void 0 ? _h : 3), 1, 3);
    ...
    const rawStep = Math.max(1, Math.floor(((_l = (_k = (_j = stage.cards) === null || _j === void 0 ? void 0 : _j.rules) === null || _k === void 0 ? void 0 : _k.ammoStep) !== null && _l !== void 0 ? _l : DEFAULT_AMMO_STEP)));
    ...
    let ammoMinPref = Math.max(1, Math.floor((_m = meta.ammoMin) !== null && _m !== void 0 ? _m : 10));
    let ammoMaxPref = Math.max(1, Math.floor((_o = meta.ammoMax) !== null && _o !== void 0 ? _o : AMMO_HARD_MAX));
    ammoMaxPref = Math.min(ammoMaxPref, AMMO_HARD_MAX);
    if (ammoMinPref > ammoMaxPref) {
        try {
            console.warn('[StageCompiler] Stage ' +
                id +
                ': ammoMin(' +
                ammoMinPref +
                ') > ammoMax(' +
    ...
    const layerCount = (0, util_1.clamp)(Math.floor((_p = meta.layerCount) !== null && _p !== void 0 ? _p : 1), 1, 5);
    // 런타임이 1겹인데 툴에서 레이어를 쓰면 바로 깨지므로 막아두는 게 안전합니다.
    if (!enableStacks) {
        if (layerCount > 1) {
            throw new Error(`Stage ${id}: layerCount=${layerCount}. 현재 런타임은 1겹만 지원합니다(겹 런타임 패치 필요).`);
        }
        if (objects.some((o) => o.layer && o.layer > 0)) {
            throw new Error(`Stage ${id}: object.layer>0 발견. 현재 런타임은 1겹만 지원합니다(겹 런타임 패치 필요).`);
        }
    }
    const seed = ((_q = meta.seed) !== null && _q !== void 0 ? _q : 1000 + id * 777) | 0;
    const rng = (0, util_1.mulberry32)(seed);
    const paletteHex = (0, colors_1.makePaletteHex)(colorCount);
    // --- 1) 보드 생성: boardTop + (옵션) stacks ---
    const boardTop = Array.from({ length: BOARD }, () => new Array(BOARD).fill(-1));
    const hiddenMask = Array.from({ length: BOARD }, () => new Array(BOARD).fill(false));
    const special = Array.from({ length: BOARD }, () => new Array(BOARD).fill(null));
    const largeMask = Array.from({ length: BOARD }, () => new Array(BOARD).fill(false));
    const largeBlocks = [];
    // stacksTemp[y][x][layer] = colorId or -1
    const stacksTemp = enableStacks
        ? Array.from({ length: BOARD }, () => Array.from({ length: BOARD }, () => new Array(layerCount).fill(-1)))
        : null;
    const putCell = (x, y, colorId, layer) => {
        if (x < 0 || x >= BOARD || y < 0 || y >= BOARD)
            return;
        const cid = ((colorId % colorCount) + colorCount) % colorCount;
        if (enableStacks && stacksTemp) {
            const L = (0, util_1.clamp)(Math.floor(layer), 0, layerCount - 1);
            stacksTemp[y][x][L] = cid;
            // top 갱신
            let top = -1;
            for (let l = 0; l < layerCount; l++)
                if (stacksTemp[y][x][l] >= 0)
                    top = stacksTemp[y][x][l];
            boardTop[y][x] = top;
        }
        else {
            boardTop[y][x] = cid;
    ...
    const inBounds = (x, y) => x >= 0 && x < BOARD && y >= 0 && y < BOARD;
    ...
    for (const obj of objects) {
        const layer = (0, util_1.clamp)(Math.floor((_r = obj.layer) !== null && _r !== void 0 ? _r : 0), 0, layerCount - 1);
        switch (obj.type) {
            case 'BLOCK_NORMAL':
            case 'BLOCK_SMALL': {
                putCell(obj.x, obj.y, (0, colors_1.normalizeColorIdAny)(obj.color), layer);
                break;
            }
            case 'BLOCK_LARGE': {
                const cid = (0, colors_1.normalizeColorIdAny)(obj.color);
                const hp = Math.max(1, Math.floor((_s = obj.hp) !== null && _s !== void 0 ? _s : 1));
                ...
                for (let dy = 0; dy < 2; dy++) {
                    for (let dx = 0; dx < 2; dx++) {
                        ...
                        const yy = (obj.y | 0) + dy;
                        if (inBounds(xx, yy))
                            largeMask[yy][xx] = true;
                    }
                }
                break;
            }
            case 'BLOCK_LARGE_HIDDEN': {
                const cid = obj.color != null ? (0, colors_1.normalizeColorIdAny)(obj.color) : (0, util_1.rngInt)(rng, 0, colorCount);
                const hp = Math.max(1, Math.floor((_t = obj.hp) !== null && _t !== void 0 ? _t : 1));
            ...
            case 'CHAIN_BARRIER': {
                ...
                if (!inBounds(x0, y0))
                    break;
                const chainIdRaw = String((_v = (_u = obj.chainId) !== null && _u !== void 0 ? _u : obj.chainGroupId) !== null && _v !== void 0 ? _v : '').trim();
                const chainOrder = Number.isFinite(obj.chainOrder)
                    ? (obj.chainOrder | 0)
                    : undefined;
                const chainLen = Number.isFinite(obj.chainLen) ? (obj.chainLen | 0) : undefined;
                chainBarriers.push({ x: x0, y: y0, layer, color: obj.color, chainIdRaw, chainOrder, chainLen });
                break;
            }
            case 'KEY': {
                putCell(obj.x, obj.y, (0, util_1.rngInt)(rng, 0, colorCount), layer);
                const kid = String((_w = obj.keyId) !== null && _w !== void 0 ? _w : '').trim();
                if (!kid)
                    throw new Error(`Stage ${id}: KEY id is empty at (${obj.x},${obj.y})`);
                if (inBounds(obj.x, obj.y))
                    special[obj.y][obj.x] = { t: 'KEY', id: kid };
                keyIds.add(kid);
                break;
            }
            case 'LOCK': {
                putCell(obj.x, obj.y, (0, util_1.rngInt)(rng, 0, colorCount), layer);
                const lid = String((_x = obj.lockId) !== null && _x !== void 0 ? _x : '').trim();
                if (!lid)
                    throw new Error(`Stage ${id}: LOCK id is empty at (${obj.x},${obj.y})`);
                if (inBounds(obj.x, obj.y))
                    special[obj.y][obj.x] = { t: 'LOCK', id: lid };
                lockIds.add(lid);
                break;
            }
            case 'PILLAR': {
                ...
                const x0 = obj.x | 0;
                const y0 = obj.y | 0;
                const h0 = Math.max(1, Math.floor((_z = (_y = obj.h) !== null && _y !== void 0 ? _y : obj.height) !== null && _z !== void 0 ? _z : 1));
                // 유효 범위 내로만 클램프
                if (x0 < 0 || x0 >= BOARD)
                    break;
                if (y0 < 0 || y0 >= BOARD)
            ...
            case 'SPAWNER_BOX': {
                ...
                if (!(x0 >= 0 && x0 + 1 < BOARD && y0 >= 1 && y0 + 1 < BOARD))
                    break;
                const spawnBlocks = Math.max(0, Math.floor((_1 = (_0 = obj.hp) !== null && _0 !== void 0 ? _0 : obj.spawnCount) !== null && _1 !== void 0 ? _1 : 0));
                if (spawnBlocks <= 0)
                    break;
                // ✅ v48+: poolColors는 "좌/우 고정 색"으로 해석합니다.
                // - poolColors[0] = 왼쪽 셀( x0 )
                // - poolColors[1] = 오른쪽 셀( x0+1 )
                const poolColors = Array.isArray((_2 = obj.spawn) === null || _2 === void 0 ? void 0 : _2.poolColors)
                    ? obj.spawn.poolColors
                    : [];
                const poolIdsRaw = poolColors.length
                    ? poolColors.map((c) => (0, colors_1.normalizeColorIdAny)(c)).filter((v) => v >= 0 && v < colorCount)
                    : [];
                const poolIds = poolIdsRaw.length ? poolIdsRaw : [0, 0];
                const leftColor = ((_3 = poolIds[0]) !== null && _3 !== void 0 ? _3 : 0) | 0;
                const rightColor = ((_5 = (_4 = poolIds[1]) !== null && _4 !== void 0 ? _4 : poolIds[0]) !== null && _5 !== void 0 ? _5 : 0) | 0;
    ...
    if (chainBarriers.length > 0) {
        ...
        for (const arr of byRow.values()) {
            ...
            for (const o of arr) {
                const id0 = String((_6 = o.chainIdRaw) !== null && _6 !== void 0 ? _6 : '').trim();
                if (!id0)
                    continue;
                idCount.set(id0, ((_7 = idCount.get(id0)) !== null && _7 !== void 0 ? _7 : 0) + 1);
            }
            ...
            if (hasRepeatId) {
                ...
                const byId = new Map();
                for (const o of arr) {
                    const id0 = String((_8 = o.chainIdRaw) !== null && _8 !== void 0 ? _8 : '').trim() || `C_${o.layer}_${o.x | 0}_${o.y | 0}`;
                    let g = byId.get(id0);
                    if (!g)
                        byId.set(id0, (g = []));
                    g.push(o);
                ...
                for (const [chainId, list] of byId) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        const o = list[i];
                        const order = (o.chainOrder != null ? o.chainOrder : i) | 0;
                        const cid = (0, colors_1.normalizeColorIdAny)(o.color);
                ...
                const anyLenGt1 = arr.some((o) => { var _a; return (((_a = o.chainLen) !== null && _a !== void 0 ? _a : 0) | 0) > 1; });
                if (!anyLenGt1) {
                    // (B-2) run 기반(연속된 x) 그룹핑
                    const parseNum = (s) => {
                        const m = /(\d+)(?!.*\d)/.exec(String(s !== null && s !== void 0 ? s : ''));
                        if (!m)
                            return null;
                    ...
                    for (const head of arr) {
                        ...
                        if (visited.has(hx))
                            continue;
                        const chainId = String((_9 = head.chainIdRaw) !== null && _9 !== void 0 ? _9 : '').trim() || `C_${head.layer}_${hx}_${head.y | 0}`;
                        // 호환 처리:
                        // - 최신 데이터: chainLen은 "그룹 길이(1~10)" 의미(= head 포함)
                        // - 구형/오염 데이터: 값이 이상해도 최소 1칸은 보장
                        let groupSize = head.chainLen != null ? (head.chainLen | 0) : 1;
                        ...
                        for (let k = 0; k < groupSize; k++) {
                            ...
                            if (!cell) {
                                if (!inBounds(xx, head.y))
                                    break;
                                if (visited.has(xx))
                                    continue;
                                if (largeMask[head.y][xx])
                                    break;
                                if (special[head.y][xx] != null)
                                    break;
                                if (boardTop[head.y][xx] >= 0)
                                    break;
                                visited.add(xx);
                                const cid = (0, colors_1.normalizeColorIdAny)(head.color);
                            ...
                            if (visited.has(xx))
                                continue;
                            visited.add(xx);
                            const cid = (0, colors_1.normalizeColorIdAny)(cell.color);
    ...
    if (enableStacks && stacksTemp) {
        for (let y = 0; y < BOARD; y++) {
            for (let x = 0; x < BOARD; x++) {
                const sp = (_11 = (_10 = special[y]) === null || _10 === void 0 ? void 0 : _10[x]) !== null && _11 !== void 0 ? _11 : null;
                if ((sp === null || sp === void 0 ? void 0 : sp.t) === 'KEY' || (sp === null || sp === void 0 ? void 0 : sp.t) === 'LOCK')
                    continue;
                for (let l = 0; l < layerCount; l++) {
        ...
        for (let y = 0; y < BOARD; y++)
            for (let x = 0; x < BOARD; x++) {
                const sp = (_13 = (_12 = special[y]) === null || _12 === void 0 ? void 0 : _12[x]) !== null && _13 !== void 0 ? _13 : null;
                if ((sp === null || sp === void 0 ? void 0 : sp.t) === 'KEY' || (sp === null || sp === void 0 ? void 0 : sp.t) === 'LOCK')
                    continue;
    ...
    const supplyPagesRaw = Array.isArray((_14 = stage.supply) === null || _14 === void 0 ? void 0 : _14.pages) ? stage.supply.pages : [];
    // (레거시) perCol 방식: 컬럼별 공급 개수(색은 컴파일러가 seed로 생성)
    const perColRaw = Array.isArray((_15 = stage.supply) === null || _15 === void 0 ? void 0 : _15.perCol) ? stage.supply.perCol : [];
    const perCol = Array.from({ length: BOARD }, (_, i) => { var _a; return Math.max(0, Math.floor((_a = perColRaw[i]) !== null && _a !== void 0 ? _a : 0)); });
    // supplySumHits: "추가로 필요한 타수" 기준 합계
    // - SMALL/HIDDEN: 1개 = 1타
    // - LARGE(2x2): hp 만큼(footprint 4와 무관)
    let supplySumHits = 0;
    // --- 3) columnSupply 생성(색 히스토그램 포함) ---
    const columnSupply = Array.from({ length: BOARD }, () => []);
    // ✅ supply에서 LARGE(2x2)를 런타임에서 BIG 1개로 생성하기 위해
    //    (idx*4+cell) 토큰을 columnSupply에 넣고, 정의는 별도 배열로 보관합니다.
    const supplyLargeDefs = [];
    const histSupply = new Array(colorCount).fill(0);
    if (supplyPagesRaw.length > 0) {
        // pages -> columnSupply
        for (let p = 0; p < supplyPagesRaw.length; p++) {
            const pageObjs = ((_17 = (_16 = supplyPagesRaw[p]) === null || _16 === void 0 ? void 0 : _16.objects) !== null && _17 !== void 0 ? _17 : []);
            // 컬럼별로 (y,cid) 모아서 y 오름차순(=아래→위)으로 정렬한 뒤 append
            const colTuples = Array.from({ length: BOARD }, () => []);
            for (let oi = 0; oi < pageObjs.length; oi++) {
                const obj = pageObjs[oi];
                const t = String((_18 = obj === null || obj === void 0 ? void 0 : obj.type) !== null && _18 !== void 0 ? _18 : '');
                const x = ((_19 = obj === null || obj === void 0 ? void 0 : obj.x) !== null && _19 !== void 0 ? _19 : -1) | 0;
                const y = ((_20 = obj === null || obj === void 0 ? void 0 : obj.y) !== null && _20 !== void 0 ? _20 : -1) | 0;
                if (x < 0 || x >= BOARD || y < 0 || y >= BOARD)
                    continue;
                if (t === 'BLOCK_NORMAL' || t === 'BLOCK_SMALL') {
                    ...
                    const cid = obj.color != null
                        ? (0, colors_1.normalizeColorIdAny)(obj.color)
                        : (0, util_1.rngInt)((0, util_1.mulberry32)((seed + 9000 + p * 97 + x * 13 + y * 101) | 0), 0, colorCount);
                    ...
                    const cid0 = obj.color != null
                        ? (0, colors_1.normalizeColorIdAny)(obj.color)
                        : (0, util_1.rngInt)((0, util_1.mulberry32)((seed + 9300 + p * 97 + x * 13 + y * 101) | 0), 0, colorCount);
                    const hp = Math.max(1, Math.floor((_21 = obj.hp) !== null && _21 !== void 0 ? _21 : 1));
                    ...
                    for (let dy = 0; dy < 2; dy++) {
                        for (let dx = 0; dx < 2; dx++) {
                            ...
                            const enc = (Types_1.SUPPLY_LARGE_BASE + idx * 4 + cell) | 0;
        ...
        for (let c = 0; c < colorCount; c++)
            if (histInitial[c] > 0 || histLargeNeed[c] > 0)
                presentColors.push(c);
        if (presentColors.length === 0)
            presentColors = Array.from({ length: colorCount }, (_, i) => i);
        const presentWeights = presentColors.map((cid) => Math.max(1, histInitial[cid] + histLargeNeed[cid]));
        const rngSupply = (0, util_1.mulberry32)(seed + 31);
        for (let col = 0; col < BOARD; col++) {
            const n = (_22 = perCol[col]) !== null && _22 !== void 0 ? _22 : 0;
            const dropOrder = [];
            for (let i = 0; i < n; i++) {
                let sum = 0;
                for (const w of presentWeights)
                    sum += Math.max(0, w);
                let cid = presentColors[(0, util_1.rngInt)(rngSupply, 0, presentColors.length)];
                if (sum > 0) {
                    let r = rngSupply() * sum;
    ...
    const blockCountMeta = Math.max(0, Math.floor((_24 = (_23 = stage.meta) === null || _23 === void 0 ? void 0 : _23.blockCount) !== null && _24 !== void 0 ? _24 : 0));
    ...
    if (supplyNeeded > 0) {
        ...
        for (let y = 0; y < BOARD; y++) {
            for (let x = 0; x < BOARD; x++) {
                const ent = (_25 = boardTop[y]) === null || _25 === void 0 ? void 0 : _25[x];
                if (!ent)
                    continue;
                // KEY/LOCK 등은 카운트에서 제외하므로 supply 색 소스로도 제외합니다.
                const sp = ent.special;
                const st = sp === null || sp === void 0 ? void 0 : sp.t;
                if (st === 'KEY' || st === 'LOCK')
                    continue;
                const cid = ent.colorId;
                if (typeof cid === 'number' && cid >= 0 && cid < colorCount)
                    fillColors.push(cid | 0);
            }
        }
        // 2) 공급페이지에서도 색을 추가(있으면)
        if ((_27 = (_26 = stage.supply) === null || _26 === void 0 ? void 0 : _26.pages) === null || _27 === void 0 ? void 0 : _27.length) {
            for (let p = 0; p < stage.supply.pages.length; p++) {
                const objs = ((_29 = (_28 = stage.supply.pages[p]) === null || _28 === void 0 ? void 0 : _28.objects) !== null && _29 !== void 0 ? _29 : []);
                for (let i = 0; i < objs.length; i++) {
                    const o = objs[i];
                    const t = o === null || o === void 0 ? void 0 : o.type;
                    if (!t)
                        continue;
                    if (t === 'KEY' || t === 'PILLAR')
                        continue;
                    const cid = typeof o.color === 'string' ? (0, Types_1.colorIdFromToken)(o.color, 12) : ((_30 = o.colorId) !== null && _30 !== void 0 ? _30 : -1);
                    if (typeof cid === 'number' && cid >= 0 && cid < colorCount)
                        fillColors.push(cid | 0);
                }
            }
        ...
        if (recomputed > blockCount)
            blockCount = recomputed;
    }
    // supply 합계(디버그용)
    ...
    if (totalBlocks < requiredHits)
        totalBlocks = requiredHits;
    // --- 4) 카드 생성:
    ...
    const totalAmmoRule = (_32 = (_31 = stage.cards) === null || _31 === void 0 ? void 0 : _31.rules) === null || _32 === void 0 ? void 0 : _32.totalAmmoEquals;
    // ✅ 총탄 목표는 "블럭 총 개수"(툴 meta.blockCount)를 우선 사용합니다.
    // (totalBlocks는 CHAIN 추가히트 등으로 보정된 "승리/남은블럭" 기준이라, 카드 총탄과 분리)
    const baseAmmo = totalAmmoRule === 'BOARD_BLOCKS' || totalAmmoRule == null
        ? blockCount
    ...
    if (totalAmmoTarget > 0) {
        if (totalAmmoTarget % 10 !== 0 && totalAmmoTarget % 5 === 0)
            ammoStep = 5;
        else
    ...
    const shot2Rate = clamp01((_33 = meta.shot2Rate) !== null && _33 !== void 0 ? _33 : 0.3);
    ...
    const buildCardsForColor = (colorId, total, rngLocal) => {
        var _a;
        const T = total | 0;
        if (T <= 0)
            return [];
        ...
        if (T < minPref) {
            const shotCount = rngLocal() < shot2Rate ? 2 : 1;
        ...
        count = (0, util_1.clamp)(count, Math.max(1, minCards), Math.max(1, maxCards));
        // step(5/10) 우선: 총합이 10으로 나눠지면 10, 아니면 5, 아니면 1
        const step = T % 10 === 0 ? 10 : T % 5 === 0 ? 5 : 1;
        // 1) shotCount 결정(결정성 유지)
        const shot = new Array(count);
        for (let i = 0; i < count; i++)
            shot[i] = rngLocal() < shot2Rate ? 2 : 1;
        ...
        for (let i = 0; i < count && remain > 0; i++) {
            ...
            const target = (0, util_1.clamp)(pref, Math.max(hardMin, minPref), hardMax);
            let need = target - ammo[i];
            if (need <= 0)
                continue;
            if (step > 1)
                need = (0, util_1.roundDownToStep)(need, step);
        ...
        while (remain > 0 && guard-- > 0) {
            ...
            if (inc > remain)
                inc = remain;
            if (step > 1 && inc === remain && remain % step !== 0)
                inc = 1;
            if (ammo[pick] + inc > hardMax)
                inc = hardMax - ammo[pick];
            if (inc <= 0)
                break;
            ammo[pick] += inc;
            remain -= inc;
        ...
        if (diff !== 0) {
            ...
            for (let i = 0; i < count; i++)
                sum2 += ammo[i] | 0;
            if (sum2 !== T) {
                throw new Error(`Stage ${id}: ammo sum adjust failed color=${(_a = Types_1.COLOR_TOKEN_ORDER[colorId]) !== null && _a !== void 0 ? _a : colorId} expected=${T} got=${sum2}`);
            }
        }
        const out = new Array(count);
        for (let i = 0; i < count; i++)
            out[i] = { colorId, ammo: ammo[i] | 0, shotCount: shot[i] };
        return out;
    };
    // --- 4-1) 카드 소스 결정 ---
    ...
    const cardsMode = ((_35 = (_34 = stage.cards) === null || _34 === void 0 ? void 0 : _34.mode) !== null && _35 !== void 0 ? _35 : 'GENERATE');
    const authored = (_36 = stage.cards) === null || _36 === void 0 ? void 0 : _36.authored;
    const list = (_37 = stage.cards) === null || _37 === void 0 ? void 0 : _37.list;
    const src = (_38 = (Array.isArray(authored) && authored.length > 0 ? authored : undefined)) !== null && _38 !== void 0 ? _38 : (Array.isArray(list) && list.length > 0 ? list : undefined);
    ...
    const preserveAuthoredOrder = isAuthoredMode && !!src;
    {
        if (isAuthoredMode && src) {
            for (let i = 0; i < src.length; i++) {
                const it = (_39 = src[i]) !== null && _39 !== void 0 ? _39 : {};
                const rawColor = ((_40 = it.color) !== null && _40 !== void 0 ? _40 : it.colorId);
                let colorId = -1;
                if (typeof rawColor === 'number')
                    colorId = rawColor | 0;
                else if (typeof rawColor === 'string')
                    colorId = (0, Types_1.colorIdFromToken)(rawColor, 12);
                // 범위 밖이면 스킵(데이터 오류 방어)
                if (colorId < 0 || colorId >= colorCount)
                    continue;
                const ammo = (0, util_1.clamp)(((_41 = it.ammo) !== null && _41 !== void 0 ? _41 : 0) | 0, AMMO_HARD_MIN, AMMO_HARD_MAX);
                if (ammo <= 0)
                    continue;
                // shotCount가 누락된 authored 데이터가 들어올 수 있어, ammo 기반으로 보정합니다.
                // - 기본 규칙: ammo>=30이면 2총구, 아니면 1총구
                const sc = ((_42 = it.shotCount) !== null && _42 !== void 0 ? _42 : undefined);
                const inferred = ammo >= 30 ? 2 : 1;
                const shotCount = sc === 1 || sc === 2 ? sc : inferred;
                // ✅ 힌트/표시 전용 필드는 authored에서 그대로 보존합니다.
                const kind = ((_43 = it.kind) !== null && _43 !== void 0 ? _43 : undefined);
                const hintStep = ((_44 = it.hintStep) !== null && _44 !== void 0 ? _44 : undefined);
                const hintRank = ((_45 = it.hintRank) !== null && _45 !== void 0 ? _45 : undefined);
                const cd = { colorId, ammo, shotCount };
                if (kind != null)
                    cd.kind = kind;
                if (hintStep != null)
                    cd.hintStep = hintStep | 0;
                if (hintRank != null)
                    cd.hintRank = hintRank | 0;
                cards.push(cd);
            }
            ...
            if (cardsMode === 'AUTHORED' || cardsMode === 'FIXED') {
                ...
                for (let i = 0; i < cards.length; i++)
                    sumByColor[cards[i].colorId | 0] += cards[i].ammo | 0;
                for (let c = 0; c < colorCount; c++) {
                    ...
                    if (expected !== got) {
                        ...
                        if (!fixed) {
                            throw new Error(`Stage ${id}: authored ammo mismatch color=${(_46 = Types_1.COLOR_TOKEN_ORDER[c]) !== null && _46 !== void 0 ? _46 : c} expected=${expected} got=${got}`);
                        }
                    }
                }
            }
            ...
            for (let c = 0; c < colorCount; c++) {
                ...
                const rngLocal = (0, util_1.mulberry32)((seed + 400000 + c * 97) | 0);
    ...
    if (((_47 = stage.cards) === null || _47 === void 0 ? void 0 : _47.mode) == null || ((_48 = stage.cards) === null || _48 === void 0 ? void 0 : _48.mode) === 'GENERATE') {
        (0, cards_1.ensureMinCardCountSoft)(cards, slotCount, ammoStep, AMMO_HARD_MAX);
    }
    ...
    // - 목적: 잔탄/부족탄으로 인한 데드락을 컴파일 단계에서 즉시 차단
    {
        ...
        for (let c = 0; c < colorCount; c++) {
            ...
            totalGot += got;
            if (expected !== got) {
                throw new Error(`Stage ${id}: ammo sum mismatch color=${(_49 = Types_1.COLOR_TOKEN_ORDER[c]) !== null && _49 !== void 0 ? _49 : c} expected=${expected} got=${got}`);
            }
        }
    ...
    const forcedPickerCols = (0, util_1.clamp)(Math.max(1, ((_50 = meta.pickerCols) !== null && _50 !== void 0 ? _50 : 5) | 0), 1, 5);
    const grid = (0, cards_1.decidePickerGrid)(cards.length, forcedPickerCols);
    if (grid.cols !== forcedPickerCols) {
        // decidePickerGrid가 강제 cols를 바꾸면 안 됨(안전장치)
        grid.cols = forcedPickerCols;
        grid.rows = Math.ceil(cards.length / forcedPickerCols);
    }
    // ✅ 카드(유닛) 순서 설계(요구사항)
    ...
    const firstShooterColors = [];
    {
        ...
        for (const lb of largeBlocks) {
            if ((lb.y | 0) !== 0)
                continue;
            const cid = ((_51 = lb.colorId) !== null && _51 !== void 0 ? _51 : -1) | 0;
            if (cid >= 0 && cid < colorCount && seen[cid] === 0) {
                seen[cid] = 1;
                firstShooterColors.push(cid);
            }
        ...
        // 앞쪽 1장씩 우선 노출
        if (!preserveAuthoredOrder)
            (0, cards_1.promoteColorsToFrontInPlace)(cards, firstShooterColors, 1);
    }
    ...
    if (!preserveAuthoredOrder) {
        {
            ...
            for (const lb of largeBlocks) {
                if ((lb.y | 0) !== 0)
                    continue;
                const cid = ((_52 = lb.colorId) !== null && _52 !== void 0 ? _52 : -1) | 0;
                if (cid >= 0 && cid < colorCount)
                    bottom[cid] = 1;
            }
            const idx0 = 0;
            ...
            const hasBottomColor = (cards[idx0] && bottom[cards[idx0].colorId | 0] === 1) ||
    ...
    const boardStacks = enableStacks && stacksTemp
        ? Array.from({ length: BOARD }, (_, y) => Array.from({ length: BOARD }, (_, x) => {
            const s = [];
            for (let l = 0; l < layerCount; l++) {
                const v = stacksTemp[y][x][l];
                if (v >= 0)
    ...
    const lockShooterCount = Math.max(0, Math.floor((_54 = (_53 = stage.meta) === null || _53 === void 0 ? void 0 : _53.lockShooterCount) !== null && _54 !== void 0 ? _54 : 0));
    ...
    if (!preserveAuthoredOrder) {
        {
            ...
            const indexOf = (r, c) => r * BOARD + c;
            const inBounds = (c, r) => c >= 0 && c < BOARD && r >= 0 && r < BOARD;
            // occ: 각 셀에 점유 엔티티 id(-1=빈칸)
            const occ = new Int32Array(BOARD * BOARD);
            occ.fill(-1);
            // special/hidden/stack은 시뮬 전용 복사본을 사용(원본 변형 방지)
            const specialSim = Array.from({ length: BOARD }, (_, r) => Array.from({ length: BOARD }, (_, c) => { var _a, _b; return (_b = (_a = special === null || special === void 0 ? void 0 : special[r]) === null || _a === void 0 ? void 0 : _a[c]) !== null && _b !== void 0 ? _b : null; }));
            const hiddenSim = Array.from({ length: BOARD }, (_, r) => Array.from({ length: BOARD }, (_, c) => { var _a; return ((_a = hiddenMask === null || hiddenMask === void 0 ? void 0 : hiddenMask[r]) === null || _a === void 0 ? void 0 : _a[c]) === true; }));
            // STACK 복사(각 셀 배열 복제)
            const stacksSim = boardStacks
                ? Array.from({ length: BOARD }, (_, r) => Array.from({ length: BOARD }, (_, c) => {
                    var _a;
                    const s = (_a = boardStacks === null || boardStacks === void 0 ? void 0 : boardStacks[r]) === null || _a === void 0 ? void 0 : _a[c];
                    return Array.isArray(s) ? s.slice() : [];
                }))
                : null;
            // columnSupply 복사(pop 기반)
            const supplyCopy = Array.from({ length: BOARD }, (_, c) => {
                const src = columnSupply === null || columnSupply === void 0 ? void 0 : columnSupply[c];
            ...
            if (Array.isArray(largeBlocks)) {
                for (let i = 0; i < largeBlocks.length; i++) {
                    ...
                    const e = {
                        ...
                        hpMax: Math.max(1, ((_55 = lb.hpMax) !== null && _55 !== void 0 ? _55 : lb.hp) | 0),
                        hidden: lb.hidden === true,
                        special: null,
                    };
                    ents.push(e);
                    for (let dy = 0; dy < 2; dy++) {
            ...
            for (let r = 0; r < BOARD; r++) {
                for (let c = 0; c < BOARD; c++) {
                    ...
                    if (baseColor < 0 && !sp)
                        continue;
                    const hp = (sp === null || sp === void 0 ? void 0 : sp.t) === 'CHAIN' ? Math.max(1, sp.hp | 0) : 1;
                    const e = {
                        id: nextId++,
                        alive: true,
                        kind: 'SMALL',
            ...
            const entIndexById = new Map();
            for (let i = 0; i < ents.length; i++)
                entIndexById.set(ents[i].id, i);
            // LOCK 해제 상태(시뮬 전용)
            ...
            const isShootableEnt = (e, colorId) => {
                if ((e.colorId | 0) !== (colorId | 0))
                    return false;
                // HIDDEN은 바닥(row=0)에 도달하기 전까지 발사 대상에서 제외(간단 모델)
                if (e.hidden && e.row > 0)
                    return false;
                // LARGE_HIDDEN도 동일 규칙
                if (e.kind === 'BIG' && e.hidden && e.row > 0)
                    return false;
                const sp = e.special;
                if ((sp === null || sp === void 0 ? void 0 : sp.t) === 'LOCK' && !unlocked.has(sp.id))
                    return false;
                return true;
            };
            // 바닥(row=0)에서 colorId로 타격 가능한 컬럼(좌측 우선)
            const findShootableColumn = (colorId) => {
                for (let c = 0; c < BOARD; c++) {
                    const e = entAt(0, c);
                    if (!e)
                        continue;
                    if (isShootableEnt(e, colorId))
                        return c;
                }
                return -1;
            };
            const rebuildShootableMask = (mask) => {
                mask.fill(0);
                for (let c = 0; c < BOARD; c++) {
                    const e = entAt(0, c);
                    if (!e)
                ...
                for (let dy = 0; dy < e.h; dy++) {
                    for (let dx = 0; dx < e.w; dx++) {
                        ...
                        if (inBounds(c, r)) {
                            ...
                            hiddenSim[r][c] = false;
                            if (stacksSim)
                                stacksSim[r][c].length = 0;
                        }
                    }
                }
            };
            const commitOccForEnt = (e) => {
                for (let dy = 0; dy < e.h; dy++) {
                    for (let dx = 0; dx < e.w; dx++) {
                        const r = e.row + dy;
                        const c = e.col + dx;
            ...
            const fillSupply = () => {
                for (let c = 0; c < BOARD; c++) {
                    ...
                    for (let r = BOARD - 1; r >= 0; r--) {
                        if (occ[indexOf(r, c)] >= 0)
                            continue;
                        if (supplyCopy[c].length <= 0)
                            break;
                        const nextColor = supplyCopy[c].pop();
                        const e = {
                            id: nextId++,
                            alive: true,
                            kind: 'SMALL',
            ...
            const hitAtColumn = (colHit) => {
                ...
                // 파괴 처리
                const sp = e0.special;
                if ((sp === null || sp === void 0 ? void 0 : sp.t) === 'KEY') {
                    unlocked.add(sp.id);
                }
                else if ((sp === null || sp === void 0 ? void 0 : sp.t) === 'SPAWNER') {
                    // 스포너 큐를 해당 컬럼 supply에 주입
                    const q = Array.isArray(sp.queue) ? sp.queue : [];
                    if (q.length) {
                        // pop 기반이므로, queue 순서 유지 목적이면 역순 push
                        for (let i = q.length - 1; i >= 0; i--)
                            supplyCopy[colHit].push(q[i] | 0);
                    }
                }
                // 엔티티 제거
            ...
            const simHintActions = [];
            const simHintCardOrder = [];
            ...
            const makeFreshState = () => {
                ...
                const entsSnap = ents.map((e) => ({
                    ...
                    special: e.special ? { ...e.special } : null,
                }));
                const supplySnap = supplyCopy.map((a) => a.slice(0));
                // ✅ 역할: LOCK 해제 상태(키 획득)를 재시뮬레이션(iter)마다 원복하기 위한 스냅샷
                // - 타입: KEY/LOCK id는 string 입니다.
                const unlockedSnap = new Set(unlocked);
                const restore = () => {
                    occ.set(occSnap);
                    ents.length = 0;
                    for (let i = 0; i < entsSnap.length; i++)
                        ents.push(entsSnap[i]);
                    supplyCopy.length = 0;
                    for (let i = 0; i < supplySnap.length; i++)
                        supplyCopy.push(supplySnap[i]);
                    unlocked.clear();
            ...
            const pickDesiredShootableColor = () => {
                for (let c = 0; c < colorCount; c++)
                    if (shootable[c])
                        return c;
                return -1;
            };
            const hasShootableCardInVisible = (shooters, wait) => {
            ...
            const injectDesiredColorIntoWait = (desiredColor, deckPos, wait) => {
                if (desiredColor < 0)
                    return false;
                let src = -1;
                ...
                if (dst < 0)
                    dst = wait.length > 0 ? wait[wait.length - 1] | 0 : -1;
                if (dst < 0)
                    return false;
                const tmp = cards[dst];
                cards[dst] = cards[src];
            ...
            for (let fixIter = 0; fixIter < MAX_FIX_ITERS; fixIter++) {
                ...
                for (let i = 0; i < cards.length; i++)
                    remAmmo[i] = cards[i].ammo | 0;
                const shooters = new Int32Array(slotCount);
                for (let i = 0; i < slotCount; i++)
                    shooters[i] = -1;
                const waitArr = [];
                let deckPos = 0;
                for (let i = 0; i < slotCount && deckPos < cards.length; i++)
                    shooters[i] = deckPos++;
                while (waitArr.length < waitCap && deckPos < cards.length)
                    waitArr.push(deckPos++);
                const wait = new Int32Array(waitArr);
                const actions = [];
                const usedCards = [];
                const log = (msg) => {
                    if (actions.length < MAX_LOG_ACTIONS)
                        actions.push(msg);
                };
                const normalizeWait = () => {
                    ...
                    for (let i = 0; i < wait.length - 1; i++) {
                        if ((wait[i] | 0) >= 0)
                            continue;
                        let j = i + 1;
                        while (j < wait.length && (wait[j] | 0) < 0)
                            j++;
                        if (j >= wait.length)
                            break;
                        wait[i] = wait[j];
                ...
                const isCleared = () => {
                    for (let i = 0; i < ents.length; i++)
                        if (ents[i].alive)
                            return false;
                    for (let c = 0; c < supplyCopy.length; c++)
                        if (supplyCopy[c].length > 0)
                            return false;
                    return true;
                };
                ...
                if (!deadlock) {
                    ...
                    for (let i = 0; i < actions.length; i++)
                        simHintActions.push(actions[i]);
                    simHintCardOrder.length = 0;
                    for (let i = 0; i < usedCards.length; i++)
                        simHintCardOrder.push(usedCards[i]);
                    break;
                }
                ...
                if (deadlockDesiredColor >= 0) {
                    (0, cards_1.promoteColorIntoShootersInPlace)(cards, deadlockDesiredColor, slotCount);
                }
            }
            ...
            if (simHintCardOrder.length > 0) {
                for (let k = 0; k < simHintCardOrder.length && k < 4; k++) {
                    ...
                    for (let j = 0; j < simHintCardOrder.length; j++) {
                        ...
                        if (v === k)
                            simHintCardOrder[j] = idx;
                        else if (v === idx)
                            simHintCardOrder[j] = k;
                    }
                }
            }
    ...
    const equipOrder = stage === null || stage === void 0 ? void 0 : stage.equipOrder;
    const clearHintCardOrder = [];
    if (Array.isArray(equipOrder) && equipOrder.length > 0) {
        const used = new Set();
        for (let k = 0; k < equipOrder.length; k++) {
            const want = String((_56 = equipOrder[k]) !== null && _56 !== void 0 ? _56 : '').trim();
            if (!want)
                continue;
            let found = -1;
            for (let i = 0; i < cards.length; i++) {
                ...
                const sc = ((_57 = cd.shotCount) !== null && _57 !== void 0 ? _57 : (cd.ammo >= 30 ? 2 : 1)) === 2 ? 2 : 1;
                const token = `${(_58 = Types_1.COLOR_TOKEN_ORDER[cd.colorId]) !== null && _58 !== void 0 ? _58 : cd.colorId}${sc}`;
                if (token === want) {
                    found = i;
                    break;
                }
        ...
        for (let i = 0; i < simClearCardOrder.length && i < 10; i++)
            clearHintCardOrder.push(simClearCardOrder[i] | 0);
    }
    else {
        for (let i = 0; i < cards.length && i < 10; i++)
            clearHintCardOrder.push(i);
    }
    ...
    if (preserveAuthoredOrder) {
        ...
        for (let i = 0; i < cards.length; i++) {
            const d = cards[i];
            const v = ((_59 = d === null || d === void 0 ? void 0 : d.hintStep) !== null && _59 !== void 0 ? _59 : 0) | 0;
            if (v > 0) {
                hasHint = true;
                break;
            }
    ...
    return {
        ...
        supplyLargeDefs: supplyLargeDefs.length ? supplyLargeDefs : undefined,
